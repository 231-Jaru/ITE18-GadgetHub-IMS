...
<head>
    ...
    <title>New Transaction - Gadgethub</title>
...
<body>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="TransactionType" name="TransactionType" required>
                                            <option value="IN" {{ old('TransactionType', 'IN') == 'IN' ? 'selected' : '' }}>IN (Restock - Add Stock)</option>
                                        </select>
                                        <small class="form-text text-muted">
                                            <i class="fas fa-info-circle"></i> 
                                            <strong>IN</strong> = Restocking inventory (Admin only)
                                        </small>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="datetime-local" class="form-control" id="TransactionDate" name="TransactionDate" value="{{ old('TransactionDate', now()->format('Y-m-d\TH:i')) }}" required>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="GadgetID" name="GadgetID" required>
                                            ...
                                            @foreach($gadgets as $gadget)
                                                <option value="{{ $gadget->GadgetID }}" {{ old('GadgetID') == $gadget->GadgetID ? 'selected' : '' }}>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" step="0.01" class="form-control" id="TotalAmount" name="TotalAmount" value="{{ old('TotalAmount') }}" required>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="AdminID" name="AdminID">
                                            <option value="">Current Admin (You)</option>
                                            @foreach($admins as $admin)
                                                <option value="{{ $admin->AdminID }}" {{ old('AdminID') == $admin->AdminID ? 'selected' : '' }}>
                                                    {{ $admin->Username ?? $admin->AdminName ?? 'Admin #' . $admin->AdminID }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <small class="form-text text-muted">
                                            <i class="fas fa-info-circle"></i> Leave empty to use your admin account
                                        </small>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        // Transaction type is always IN (inventory purchases only)
        const transactionTypeSelect = document.getElementById('TransactionType');
        if (transactionTypeSelect) {
            transactionTypeSelect.value = 'IN';
            transactionTypeSelect.setAttribute('readonly', 'readonly');
        }
